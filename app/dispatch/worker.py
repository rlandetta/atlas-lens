...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m app.dispatch.worker",
        description="Revisa y procesa despachos programados de DISPATCH.",
    )
...
def run_process_due(*, dry_run: bool, out: TextIO) -> int:
    ...
    with app.app_context():
        ...
        for shipment in review.due:
            ...
            scheduled_at = str(shipment.get("scheduled_at", ""))
            if dry_run:
                print(f"- DRY-RUN: {shipment_id} | método: {method}", file=out)
                continue
            started_at = datetime.now(timezone.utc).isoformat()
            # claim_for_execution atomically flips Programado -> Enviando under a
            # file lock, so a concurrent worker run cannot claim the same shipment twice.
            ...
            try:
                if method == "download_link":
                    result_shipment = prepare_download_link_delivery(claimed)
                elif method == "download_link_email":
                    result_shipment = prepare_download_link_email_delivery(claimed)
                else:
                ...
                print(
                    f"dispatch_id={shipment_id} scheduled_at={scheduled_at} started_at={started_at} "
                    f"result=ERROR sent_at= error={error}",
                    file=out,
                )
                continue
            # Some delivery helpers (e.g. SMTP send failures) catch their own
            # errors and return the shipment already marked Error instead of
            # raising, so the outcome must be checked explicitly here too.
            expected_status = "Enviado" if method == "download_link_email" else "Listo"
            if str(result_shipment.get("status", "")) != expected_status:
                failed += 1
                error_message = str(result_shipment.get("last_error", "") or "Error de envío sin detalle.")
                print(f"- ERROR: {shipment_id} | {error_message}", file=out)
                print(
                    f"dispatch_id={shipment_id} scheduled_at={scheduled_at} started_at={started_at} "
                    f"result=ERROR sent_at= error={error_message}",
                    file=out,
                )
                continue
            processed += 1
            ...
            print(
                f"dispatch_id={shipment_id} scheduled_at={scheduled_at} started_at={started_at} "
                f"result=ENVIADO sent_at={sent_at} error=",
                file=out,
            )
        print(
            "Resumen: "
