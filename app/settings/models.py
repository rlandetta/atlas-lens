...
def validate_port(value: Any) -> int:
    try:
        ...
        raise SettingsValidationError("El puerto SMTP debe ser numérico.") from error
    if port < 1 or port > 65535:
        raise SettingsValidationError("El puerto SMTP debe estar entre 1 y 65535.")
    return port
...
def normalize_channel(channel: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["sender_email"] = validate_email(normalized.get("sender_email", ""), "El correo remitente")
    ...
    normalized["smtp_port"] = validate_port(normalized.get("smtp_port", ""))
    ...
    if not normalized["credential_ref"]:
